...
onchange=\"window.location='chat2.html?q='+this.value\">";
    echo "<option value=''>";
    ...
    $id = $_SESSION['id'];
    $sql3 = "SELECT clase.Id_Profesor 
FROM repartizare 
INNER JOIN clase ON repartizare.id_clasa=clase.Id
INNER JOIN courses ON clase.Id_Materie=courses.id
WHERE repartizare.id_user=? AND clase.Nume=?";
...
WHERE clase.Nume=? AND repartizare.id_clasa=(SELECT repartizare.id_clasa FROM repartizare WHERE repartizare.id_user=? &&
 repartizare.id_clasa=(SELECT clase.Id WHERE clase.Id_Profesor=?&& clase.Nume=?)) LIMIT 1";
    $stmt = $db->prepare($sql2);
    $stmt->bind_param("ssss", $option, $id, $idp, $option);
    $stmt->execute();
